class CreateCtgovFunctions < ActiveRecord::Migration[7.0]
  ...
  def up
    execute <<-SQL
      CREATE OR REPLACE FUNCTION ctgov.ids_for_term(character varying) RETURNS TABLE(nct_id character varying)
        ...
        SELECT DISTINCT nct_id FROM ctgov.browse_conditions WHERE downcase_mesh_term like lower($1)
        UNION
        SELECT DISTINCT nct_id FROM ctgov.browse_interventions WHERE downcase_mesh_term like lower($1)
        UNION
        SELECT DISTINCT nct_id FROM ctgov.studies WHERE lower(brief_title) like lower($1)
        UNION
        SELECT DISTINCT nct_id FROM ctgov.keywords WHERE lower(name) like lower($1)
        ;
        $_$;
      ...
      CREATE OR REPLACE FUNCTION ctgov.ids_for_org(character varying) RETURNS TABLE(nct_id character varying)
        LANGUAGE sql
      AS $$
      SELECT DISTINCT nct_id FROM ctgov.responsible_parties WHERE lower(affiliation) like lower($1)
      UNION
      SELECT DISTINCT nct_id FROM ctgov.facilities WHERE lower(name) like lower($1) or lower(city) like lower($1) or lower(state) like lower($1) or lower(country) like lower($1)
      UNION
      SELECT DISTINCT nct_id FROM ctgov.sponsors WHERE lower(name) like lower($1)
      UNION
      SELECT DISTINCT nct_id FROM ctgov.result_contacts WHERE lower(organization) like lower($1)
      ;
      $$;
      ...
      FROM ctgov.studies s
        INNER JOIN ctgov.browse_conditions         bc ON s.nct_id = bc.nct_id and bc.downcase_mesh_term  like lower($1)
        LEFT OUTER JOIN ctgov.calculated_values    cv ON s.nct_id = cv.nct_id
        LEFT OUTER JOIN ctgov.all_conditions       c  ON s.nct_id = c.nct_id
        LEFT OUTER JOIN ctgov.all_interventions    i  ON s.nct_id = i.nct_id
        LEFT OUTER JOIN ctgov.all_sponsors         sp ON s.nct_id = sp.nct_id
        LEFT OUTER JOIN ctgov.eligibilities        e  ON s.nct_id = e.nct_id
        LEFT OUTER JOIN ctgov.all_id_information   id ON s.nct_id = id.nct_id
        LEFT OUTER JOIN ctgov.all_design_outcomes  o  ON s.nct_id = o.nct_id
        LEFT OUTER JOIN ctgov.designs              d  ON s.nct_id = d.nct_id
     ...
     UNION
      ...
      FROM ctgov.studies s
        INNER JOIN ctgov.conditions                bc ON s.nct_id = bc.nct_id and bc.downcase_name like lower($1)
        LEFT OUTER JOIN ctgov.calculated_values    cv ON s.nct_id = cv.nct_id
        LEFT OUTER JOIN ctgov.all_conditions       c  ON s.nct_id = c.nct_id
        LEFT OUTER JOIN ctgov.all_interventions    i  ON s.nct_id = i.nct_id
        LEFT OUTER JOIN ctgov.all_sponsors         sp ON s.nct_id = sp.nct_id
     ...
     UNION
      ...
      FROM ctgov.studies s
        INNER JOIN ctgov.keywords k ON s.nct_id = k.nct_id and k.downcase_name like lower($1)
        LEFT OUTER JOIN ctgov.calculated_values   cv ON s.nct_id = cv.nct_id
        LEFT OUTER JOIN ctgov.all_conditions      c  ON s.nct_id = c.nct_id
        LEFT OUTER JOIN ctgov.all_interventions   i  ON s.nct_id = i.nct_id
        LEFT OUTER JOIN ctgov.all_sponsors        sp ON s.nct_id = sp.nct_id
