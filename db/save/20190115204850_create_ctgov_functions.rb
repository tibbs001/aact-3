class CreateCtgovFunctions < ActiveRecord::Migration[7.0]
  ...
  def up
    execute <<-SQL
      CREATE OR REPLACE FUNCTION ids_for_term(character varying) RETURNS TABLE(nct_id character varying)
        LANGUAGE sql
        AS $_$
      ...
      GRANT EXECUTE ON FUNCTION ids_for_term(VARCHAR) TO read_only;

      CREATE OR REPLACE FUNCTION ids_for_org(character varying) RETURNS TABLE(nct_id character varying)
        LANGUAGE sql
      AS $$
      ...
      GRANT EXECUTE ON FUNCTION ids_for_org(VARCHAR) TO read_only;

      CREATE OR REPLACE FUNCTION study_summaries_for_condition(varchar)
     ...
     UNION
      ...
      GRANT EXECUTE ON FUNCTION study_summaries_for_condition(VARCHAR) TO read_only;
  ...
  def down
    execute <<-SQL
      DROP FUNCTION ids_for_org(varchar);
      DROP FUNCTION ids_for_term(varchar);
      DROP FUNCTION study_summaries_for_condition(varchar);
    SQL
  end
